...
def build_targets(p, targets, model):
    ...
    for i, j in enumerate(model.yolo_layers):  # j: [89, 101, 113]
        ...
        gain[2:] = torch.tensor(p[i].shape)[[3, 2, 3, 2]]  # xyxy gain tensor([grid_w, grid_h, grid_w, grid_h])
        na = anchors.shape[0]  # number of anchors
        # [3] -> [3, 1] -> [3, nt]
        at = torch.arange(na).view(na, 1).repeat(1, nt)  # anchor tensor, same as .repeat_interleave(nt) # [na=3, nt]
        ...
        gxy = t[:, 2:4]  # grid xy
        gwh = t[:, 4:6]  # grid wh
        gij = (gxy - offsets).long()  # 匹配targets所在的grid cell左上角坐标，即对应的anchor模板的中心点
...
def get_yolo_layers(model):
    ...
    return [i for i, x in enumerate(bool_vec) if x]  # [82, 94, 106] for yolov3, [89, 101, 113] for yolov3-spp
