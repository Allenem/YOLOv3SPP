    ...
    for i, mdef in enumerate(modules_defs):
        ...
        if mdef["type"] == "convolutional":
            ...
            if mdef["activation"] == "leaky":
                # 除了3个predictor对应的卷积activation是linear外，其他卷积层的activation都是leaky
        ...
        # 3 + 2 + 2: layers 单个回到索引的输出，多个拼接；索引负数相对索引，正数绝对索引
        elif mdef["type"] == "route":  # [-2],  [-1,-3,-5,-6], [-1, 61]
            layers = mdef["layers"]
            filters = sum([output_filters[l + 1 if l > 0 else l] for l in layers]) # +1 因为output_filters中第一个元素是输入的通道数3
        ...
        # 3: 3个predictor之后的yolo层
        elif mdef["type"] == "yolo":
            yolo_index += 1  # 记录是第几个yolo_layer [0, 1, 2]
            stride = [32, 16, 8]  # 预测特征层对应原图的缩放比例 512/16=32, 512/32=16, 512/64=8
